...
void gauss (int npts, int job, double a, double b, double xpts[], double weights[])
{
  ...
  const double eps = 1.e-12;	// limit for accuracy 
  ...
  if (job == 3)
  {
    for (int i = 0; i < npts; i++)
    {
	double xx = M_PI / 4. * (xpts[i] + 1.0);
	xpts[i] = tan (xx);
	weights[i] = M_PI / 4. * 1. / (cos(xx) * cos(xx)) * weights[i];
    }
  }
}
